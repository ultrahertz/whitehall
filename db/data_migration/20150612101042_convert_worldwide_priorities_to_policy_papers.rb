...
CSV.open(Rails.root+"tmp/#{Time.zone.now.strftime('%F-%H-%M-%S')}-policy_paper_creation_output.csv", "wb") do |csv|
  ...
  WorldLocation.active.each do |world_location|
    ...
    summary = "This is a copy of a document that stated the worldwide priorities regarding #{world_location.name} for the 2010 to 2015 Conservative and Liberal Democrat coalition government."
